...
function csrf_token() {
	...
	// NOTE: we are not really thinking 
	// about something that is cryptographically secure,
	// but rather just unique.
	return password_hash(strval(md5(strval(uniqid(rand(), TRUE)).strval(time()))), PASSWORD_ARGON2I);
}
...
function get_stored_csrf_token(){

	return "<input id=\"csrf\" type=\"hidden\" name=\"csrf_token\" value=\"".$_SESSION["csrf_token"]."\"/>";
